param (
    [Alias('p', 'path')][string]$ServerPath, # alternate server path / path for default config
    [Alias('i')][switch]$Interactive, # launch prompt mode
    [Alias('s', 'start')][switch]$StartServer, # automatically start server
    [switch]$UpdateOxide, # automatically update oxide
    [switch]$UpdateServer, # automatically update server
    [Alias('f')][switch]$Force, # update oxide even if it's the latest version already
    [Alias('c', 'clear')][switch]$ClearManaged, # clear RustDedicated_Data/Managed/ folder in case it contains modified assemblies
    [Alias('u', 'update')][switch]$FullUpdate, # update both Oxide and server
    [switch]$ServerCFG, # emit server.cfg
    [string]$SteamCMDPath, # alternate path to the steamcmd (SteamCMD.ps1 still needs to be located in the same dir as this script)
    [switch]$SteamCMDRepo, # use steamcmd as command instead of local installation
    [string]$Hostname,
    [int]$WorldSize,
    [int]$Seed,
    [string]$Map,
